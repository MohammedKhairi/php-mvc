<div class="d-md-flex align-items-center">
    <div class="col-md-6">
        <a class="btn btn-sm btn-bg mh40 p-0 px-2" href="/cp/learning/week/add">
    ...
    foreach ($data['data'] as $d): ?>
        <li class="table-row shadow">
            ...
            <div class="col col-3" data-label="وقت البداية">
                <?= Application::$app->fun->getUTS($d['time_start'],"time") ?>
            </div>
            <div class="col col-3" data-label="وقت النهاية">
                <?= Application::$app->fun->getUTS($d['time_end'],"time") ?>
            </div>

            <div class="col col-2" data-label="العمليات">
                <div class="men-tools">
                    <div class="tools-content">
                        <a href="/cp/learning/week/edit/<?= $d['id'] ?>" class="mx-1 btn btn-sm btn-primary">تعديل</a>
                        <?php if ($d['deleted'] == 0): ?>
                            <buttonn onclick="DeleteBtnAlert('/cp/learning/week/delete/<?= $d['id'] ?>')"
                                class="mx-1 btn btn-sm btn-danger">حذف</buttonn>
                        <?php else: ?>
                            <a href="/cp/learning/week/restore/<?= $d['id'] ?>" class="mx-1 btn btn-sm btn-warning">ارجاع</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
