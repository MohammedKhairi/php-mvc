<div class="d-md-flex align-items-center">
    <div class="col-md-6">
        <a class="btn btn-sm btn-bg mh40 p-0 px-2" href="/cp/employee/add">
...
<ul class="responsive-table2 fmedium my-2">
    ...
    foreach ($data['data'] as $d): ?>
        <li class="table-row shadow">
            ...
            <div class="col col-2" data-label="المستوى">
                <?= $d['lvl'] ?>
            </div>
            <div class="col col-1" data-label="العمليات">
                <div class="menu-tools">
                    <div class="tools-content">
                        <a href="/cp/employee/edit/<?= $d['eid'] ?>" class="mx-1 btn btn-sm btn-primary">تعديل</a>
                        <?php if ($d['deleted'] == 0): ?>
                            <buttonn onclick="DeleteBtnAlert('/cp/employee/delete/<?= $d['eid'] ?>')"
                                class="mx-1 btn btn-sm btn-danger">حذف</buttonn>
                        <?php else: ?>
                            <a href="/cp/employee/restore/<?= $d['eid'] ?>" class="mx-1 btn btn-sm btn-warning">ارجاع</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
